--====================================================
-- SISTEMA CONSOLIDADO DE FARM - DELTA 2025
--====================================================
local _v = game
local _s = {
...
_btnToggle.BorderSizePixel = 2

-- 2. INTERFACE PRINCIPAL (CONSOLIDADA)
...
-- Botão: Selecionar NPC
createBtn("1. SELECIONAR ALVO", UDim2.new(0.05, 0, 0.28, 0), Color3.fromRGB(50, 50, 60), function(b)
    b.Text = "CLIQUE NO NPC..."
    local conn; conn = _p:GetMouse().Button1Down:Connect(function()
        ...
        if t and t.Parent:FindFirstChild("Humanoid") then
            ...
            b.Text = "ALVO: " .. t.Parent.Name
            conn:Disconnect()
        end
    end)
end)

-- Botão: Ir até (9 studs acima)
createBtn("2. IR ATÉ (9 STUDS): OFF", UDim2.new(0.05, 0, 0.44, 0), Color3.fromRGB(50, 50, 60), function(b)
...
-- Botão: Auto Attack
createBtn("3. AUTO ATTACK: OFF", UDim2.new(0.05, 0, 0.60, 0), Color3.fromRGB(50, 50, 60), function(b)
...
-- Botão: Fechar Tudo
createBtn("FECHAR E LIMPAR TUDO", UDim2.new(0.05, 0, 0.85, 0), Color3.fromRGB(120, 30, 30), function()
    _STATE.Atk = false
    _STATE.Farm = false
...
_CONNECTIONS["Loop"] = _s.R.Heartbeat:Connect(function()
    ...
    -- Ataque Automático com suporte para Mochila
